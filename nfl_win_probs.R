...
df_spreads <- schedule %>%
  select(game_id,home_team, away_team, home_qb, home_qb_id, away_qb, away_qb_id) %>%
  left_join(offense_full, by = c("game_id", "home_team" = "posteam")) %>%
  rename(home_epa_pp = epa_pp) %>%
  left_join(offense_full, by = c("game_id", "away_team" = "posteam")) %>%
  rename(away_epa_pp = epa_pp) %>%
  left_join(qb_data, by = c("game_id", "home_qb_id" = "id")) %>%
  rename(home_qb_epa_per_play = qb_epa,)