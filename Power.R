...
master_qb_list <- rbind(schedule %>% select(name = home_qb),schedule %>% select(name = away_qb))  %>% unique() %>% rbind(data.frame(name = c("D.Mills", "S.Sanders")))
...
qb_data <- data %>%
  ...
  filter(dropbacks_per_game > 3 | name == "S.Sanders")
...
current_qbs <- current_qbs %>% mutate(qb = ifelse(team == "ATL","K.Cousins",qb))
